...
from final_values import NODE_DATA_STORAGE_PATH
import os
...
class NodeStorage:
    def __init__(self, node_id):
        ...
        try:
            ...
            self.root = et.Element('node-storage')
            ...
            with open(self.file_path, 'r') as file:
                ...
                if len(encrypted_tree) != 0:
                    self.initial_key = encrypted_tree.split('<>')[1]
                    self.encryption_key = str(hash_function(self.initial_key))
    ...
    def update_stored_data(self):
        ...
        with open(self.file_path, 'w') as file:
            file.write(stored_data_encrypted + '<>' + self.initial_key)
    ...
    def get_db_data(self, db_id):  # get data saved in db
        ...
        for collection_elem in db_tree.findall('*'):
            ...
            items = {}
            for item_elem in collection_elem.findall('*'):
                items[item_elem.tag] = str_to_dict(item_elem.text)
            db_data[key] = items
        return db_data

    def insert_data(self, db_id, collection_name, items: [Item]):
        # get db id element
        ...
        for item in items:
            ...
            item_elem.text = dict_to_str(item.data)
        self.update_stored_data()
