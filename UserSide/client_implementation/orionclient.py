...
from userside_final_values import MAIN_SERVER_ADDRESS, NODE_COMMUNICATION_PORT, STORAGE_COPIES
...
from data_storage_implementation.database import DB
import os
...
class OrionClient:
    ...
    def __getitem__(self, db_name):  # download db to client, get from cache, or create local db
        if db_name not in self.account_dbs.keys():
    ...
    def upload_data(self, db_name, _second_attempt=False):  # upload an updated database to the system
        ...
        if len(initial_best_target_nodes) > STORAGE_COPIES:
            ...
            best_target_nodes = initial_best_target_nodes

        # isolate only collections that were requested by server
        updated_collections = packet.data['updated data']

        # send selected nodes to server
        self.server_socket.send(''.encode())
        ...
        encoded = self.server_socket.recv()
        packet = Packet(request_encoded=encoded)
        if packet.token != self.session_token:
    ...
    def _download_data(self, db_name, db_id, send_request_packet=True):
        print('download data function called')
        ...
        item_count_from_server = packet.data['item count']
        print(3)
        # receive all data
        ...
        if item_counter != item_count_from_server:
            print(12)
